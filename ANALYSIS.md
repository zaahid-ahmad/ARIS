...
**Last updated:** 2026-09-02 (latest) — Three changes: (1) Teacher Dashboard subject risk-donut cards, (2) a Year (promotion) weighting structure on top of the existing per-term weighting, surfaced as an overall percentage teachers/learners/parents can now actually see, and (3) a batched-query performance fix to `RiskAssessmentService` that removed an N+1 pattern making the Teacher Dashboard and AtRisk pages take several seconds to load. Changes marked ✅ *2026-09-02* below.
...
| 36 | `RiskAssessmentService.CalculateRiskScore`'s `AcademicAverage` still uses the same flat/unweighted average across every mark that §5.2's `CalculateYearMark` addition (2026-09-02) replaced everywhere else it appeared (`Learner/Marks.razor`, `Parent/Marks.razor`). Risk scoring — and therefore the at-risk banner, the Teacher Dashboard donut cards, and `AtRisk.razor`'s scatter chart — is not using the new weighted Term/Year marks. Deliberately not changed as part of that work: switching it would change every existing at-risk number app-wide, a separate decision | Open |
