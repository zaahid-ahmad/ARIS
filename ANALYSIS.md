...
**Verified live** end-to-end on a disposable 5-learner test scenario (built and fully torn down afterward on the `Default School` fixture, so as not to touch Riverside's persistent demo data): a matched promotion, an unmatched promotion requiring manual class selection, a repeat, and a Grade 12 graduation all produced exactly the expected `Learner`/`LearnerYearRecord`/`Subject`/`AssessmentType`/`WeightingStructure`/`WeightingNode`/`LearnerSubject` rows; the re-enrollment matching correctly handled a learner with no prior enrollment (no spurious warning); the double-run guard correctly blocked a second rollover attempt in the same run with `"This school has already been rolled over to {year}."`.
