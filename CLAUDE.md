...
dotnet user-secrets set "Email:RedirectAllTo" "<your own inbox>"   # all mail goes here; keep set with demo data
# Only for real use (never with demo data): allow sending to real recipients when RedirectAllTo is empty
# dotnet user-secrets set "Email:AllowRealRecipients" "true"
```
...
**Email verification (tracked, not enforced).** Staff-created accounts (`CreateUser.razor`, `SuperAdmin/Admins.razor`, `BulkUserImportService`) start `EmailConfirmed = false`; seeders and all pre-existing accounts stay confirmed. An address becomes verified when the user completes an emailed set-password/reset link (`ResetPassword.razor`), opens a verification link (`AccountEmailService.SendVerificationAsync` → `Account/ConfirmEmail`), or an admin clicks "Mark verified" (User Management / SuperAdmin Admins, which also show a "Not verified" badge). Sign-in is never blocked — `RequireConfirmedAccount`/`RequireConfirmedEmail` stay `false`.
