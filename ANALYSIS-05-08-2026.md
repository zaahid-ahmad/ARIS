...
These tables are queried by `LearnerId` or `AssessmentId` on every page load but have no covering index — full table scans as data grows:

| Table | Missing Index |
|---|---|
| `LearnerMarks` | `LearnerId` |
| `LearnerQuestionMarks` | `LearnerId` |
| `AssessmentQuestions` | `AssessmentId` |
| `AttendanceRecords` | `LearnerId` |
| `Interventions` | `LearnerId` |
...
| `Assessment`, `LearnerMark`, `AssessmentType` | `MaxMark`/`MarksAwarded`/`WeightPercentage` stored as `float` — rounding errors in grade calculations |
...
#### `Home.razor`
- Redirects unknown roles to `/access-denied` — **that route does not exist** (open).
...
#### `Teacher/Marks.razor` (412 lines)
- N+1 query pattern — **fixed** (batch queries applied to `LoadQuestions` and `SaveMarks`).
- Three separate `SaveChangesAsync()` calls with no transaction boundary — **fixed** (replaced by `SaveLearnerMarks()` with `BeginTransactionAsync` / single `SaveChangesAsync` / `CommitAsync`).
...
| 4 | Missing database indexes on `LearnerId` (5 tables) | Open |
| 5 | `float` used for marks and weights — switch to `decimal` | Open |
...
| 7 | Dual role storage (`User.Role` string + `IdentityRole`) | Open |
| 8 | No service interfaces — prevents unit testing | Open |
| 9 | No resource-level authorization policies | Open |
| 10 | Weak password policy — no uppercase/special required | Open |
...
| 17 | `/access-denied` route missing — `Home.razor` navigates there for unknown roles | Open |
