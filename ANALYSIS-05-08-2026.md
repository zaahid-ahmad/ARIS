...
**Last updated:** 2026-08-27 (latest) — Parent Phase 4 (admin can edit a Parent's linked children from `UserManagement.razor`), then a new `SchoolClass` entity replacing free-text `Learner.ClassName` with an admin-managed, per-grade class list (`/admin/classes`) selected via dropdown everywhere a class is assigned. Changes marked ✅ *Parent Phase 4* and ✅ *Class Management* below.
