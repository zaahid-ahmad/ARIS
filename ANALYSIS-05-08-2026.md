...
**Last updated:** 2026-08-27 (latest) — Bulk Import extended to create Parent accounts linked to already-existing learners via a new `LinkedLearnerEmails` CSV column. Changes marked ✅ *Parent Bulk Import* below.
**Previously:** 2026-08-27 — Parent Phase 4 (admin can edit a Parent's linked children from `UserManagement.razor`), then a new `SchoolClass` entity replacing free-text `Learner.ClassName` with an admin-managed, per-grade class list (`/admin/classes`) selected via dropdown everywhere a class is assigned. Changes marked ✅ *Parent Phase 4* and ✅ *Class Management* below.
