...
**Last updated:** 2026-08-27 (later same day) — New `Parent` role added end-to-end across three phases: DB schema (`Parent`/`ParentLearner`), authorization (`SchoolAuthorizationService` ownership checks), and the Parent-facing UI (Dashboard/Marks/Attendance/Overview) plus admin account-creation support. Changes marked ✅ *Parent role* below.
**Previously:** 2026-08-27 (earlier) — Teacher At-Risk scatter chart (attendance vs. academic average), Teacher Dashboard alert simplified to a click-through banner, and three double-click/race-condition crashes found and fixed across the app. Changes marked ✅ *2026-08-27* below.
